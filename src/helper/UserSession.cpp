...
bool UserSession::start() {
  ...
  qDebug() << "UserSession::start: DEBUG - XDG_SESSION_TYPE="
           << env.value(QStringLiteral("XDG_SESSION_TYPE"));
  qDebug() << "UserSession::start: DEBUG - XDG_SESSION_CLASS="
           << env.value(QStringLiteral("XDG_SESSION_CLASS"));
  qDebug() << "UserSession::start: DEBUG - m_path=" << m_path;
  qDebug() << "UserSession::start: DEBUG - m_displayServerCmd="
           << m_displayServerCmd;
  ...
  if (env.value(QStringLiteral("XDG_SESSION_TYPE")) == QLatin1String("x11")) {
    ...
    if (m_displayServerCmd.isEmpty()) {
      ...
      setArguments(args);
      qDebug() << "UserSession::start: DEBUG - starting directly with program="
               << args.first();
    } else {
      QString helperPath = QStringLiteral(LIBEXEC_INSTALL_DIR
                                          "/soniclogin-helper-start-x11user");
      setProgram(helperPath);
      setArguments({m_displayServerCmd, command});
      qDebug() << "UserSession::start: DEBUG - starting helper=" << helperPath
               << "with args=" << m_displayServerCmd << command;
    }
    ...
    if (!homeDir.isEmpty()) {
      if (QDir(homeDir).exists()) {
        ...
        qWarning() << "UserSession::start: HOME directory does not exist, "
                      "keeping default working directory:"
                   << homeDir;
      }
    }
    QProcess::start();
    qDebug() << "UserSession::start: DEBUG - QProcess::start() called, state="
             << state() << "error=" << error();
  } else if (env.value(QStringLiteral("XDG_SESSION_TYPE")) ==
  ...
  qCritical() << "UserSession::start: session process failed to start! error:"
              << error() << "-" << errorString();
  return false;
}
...
void UserSession::stop() {
  qWarning() << "UserSession::stop() CALLED - TRACE:"
             << "program=" << program() << "pid=" << processId()
             << "state=" << state() << "sessionClass="
             << processEnvironment().value(QStringLiteral("XDG_SESSION_CLASS"))
             << "sessionType="
             << processEnvironment().value(QStringLiteral("XDG_SESSION_TYPE"))
             << "user=" << processEnvironment().value(QStringLiteral("USER"));
  if (state() != QProcess::NotRunning) {
    terminate();
    const bool isGreeter =
        processEnvironment().value(QStringLiteral("XDG_SESSION_CLASS")) ==
        QLatin1String("greeter");

    // Wait longer for a session than a greeter
    if (!waitForFinished(isGreeter ? 5000 : 60000)) {
      qWarning() << "UserSession::stop() process did not finish in time, "
    ...
    qWarning() << "UserSession::stop() process not running, emitting "
                  "finished(HELPER_OTHER_ERROR)";
    Q_EMIT finished(Auth::HELPER_OTHER_ERROR);
  }
}

QString UserSession::displayServerCommand() const { return m_displayServerCmd; }

void UserSession::setDisplayServerCommand(const QString &command) {
  m_displayServerCmd = command;
}

void UserSession::setPath(const QString &path) { m_path = path; }
...
void UserSession::childModifier() {
  ...
  if (setsid() < 0) {
    qCritical("Failed to set pid %lld as leader of the new session and process "
              "group: %s",
  ...
  if (vtNumber > 0) {
    // Open TTY WITHOUT O_NOCTTY so it automatically becomes the controlling
    // terminal
  ...
  for (const QString &ns : mainConfig.Namespaces.get()) {
    ...
    if (setns(fd, 0) != 0) {
      qCritical("setns(open(%s), 0) failed: %s", qPrintable(ns),
                strerror(errno));
      exit(Auth::HELPER_OTHER_ERROR);
    }
    ::close(fd);
  ...
  QScopedPointer<char, QScopedPointerPodDeleter> buffer(
  ...
  if (setusercontext(NULL, &pw, pw.pw_uid, LOGIN_SETALL) != 0) {
    qCritical() << "setusercontext(NULL, *, " << pw.pw_uid
                << ", LOGIN_SETALL) failed for user: " << username;
    exit(Auth::HELPER_OTHER_ERROR);
  }
#else
  ...
  if (-1 == getgrouplist(pw.pw_name, pw.pw_gid, NULL, &n_user_groups)) {
    ...
    if ((n_user_groups = getgrouplist(pw.pw_name, pw.pw_gid, user_groups,
                                      &n_user_groups)) == -1) {
  ...
  if (Logind::isAvailable() && Logind::isELogind()) {
    // Set CAP_SYS_TTY_CONFIG and CAP_SETPCAP as ambient capabilities so they
    // survive setuid(). CAP_SETPCAP lets child processes modify their own
    // capability sets before execve(). prctl(PR_CAP_AMBIENT_RAISE) requires the
    // capability to be in both permitted and inheritable.
    struct __user_cap_header_struct capHeader = {_LINUX_CAPABILITY_VERSION_3,
                                                 0};
    ...
    if (capget(&capHeader, capData) == 0) {
      ...
      if (capset(&capHeader, capData) == 0) {
        if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_SYS_TTY_CONFIG, 0,
                  0) < 0) {
          qWarning() << "UserSession::childModifier: Failed to set "
                        "CAP_SYS_TTY_CONFIG ambient capability:"
                     << strerror(errno);
        } else {
          qInfo() << "UserSession::childModifier: Set CAP_SYS_TTY_CONFIG as "
                     "ambient capability";
        }
        if (prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE, CAP_SETPCAP, 0, 0) <
            0) {
          qWarning() << "UserSession::childModifier: Failed to set CAP_SETPCAP "
                        "ambient capability:"
                     << strerror(errno);
        } else {
          qInfo() << "UserSession::childModifier: Set CAP_SETPCAP as ambient "
                     "capability";
        }
        ...
        qWarning() << "UserSession::childModifier: Failed to add capabilities "
                      "to inheritable set:"
                   << strerror(errno);
      }
    } else {
      qWarning() << "UserSession::childModifier: capget failed:"
                 << strerror(errno);
    }
  }
#endif
  ...
  qInfo() << "UserSession::childModifier: setuid complete, uid=" << getuid()
          << "gid=" << getgid() << "euid=" << geteuid();

#ifdef Q_OS_LINUX
  if (Logind::isAvailable() && Logind::isELogind()) {
    int a_tty =
        prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_IS_SET, CAP_SYS_TTY_CONFIG, 0, 0);
    int a_pcap =
        prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_IS_SET, CAP_SETPCAP, 0, 0);
    qInfo() << "UserSession::childModifier: AFTER setuid ambient "
               "CAP_SYS_TTY_CONFIG="
            << a_tty << "CAP_SETPCAP=" << a_pcap;
  }
#endif
