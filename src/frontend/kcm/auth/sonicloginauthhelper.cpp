 ...
 */
#include "sonicloginauthhelper.h"
#include "config.h"
...
static std::optional<QString> sonicloginUserHomeDir()
{
    ...
    // non-existent user
    const QString sonicloginHomeDirPath = KUser("soniclogin").homeDir();
    if (sonicloginHomeDirPath.isEmpty()) {
        return std::nullopt;
    } else {
        return sonicloginHomeDirPath;
    }
}
...
{
    static const KUser sonicloginUser("soniclogin");
    chown(path.toLocal8Bit().data(), sonicloginUser.userId().nativeId(), sonicloginUser.groupId().nativeId());
}

ActionReply SonicLoginAuthHelper::sync(const QVariantMap &args)
{
    QString homeDir;
...
ActionReply SonicLoginAuthHelper::reset(const QVariantMap &args)
{
    Q_UNUSED(args);
...
ActionReply SonicLoginAuthHelper::save(const QVariantMap &args)
{
    QFile file(QLatin1String(SONICLOGIN_CONFIG_FILE));
    ...
    if (auto opt = sonicloginUserHomeDir()) {
        ...
        qWarning() << "Could not determine home directory for soniclogin user";
        return ActionReply::HelperErrorReply();
    }
...
KAUTH_HELPER_MAIN("org.kde.kcontrol.kcmsoniclogin", SonicLoginAuthHelper)

#include "moc_sonicloginauthhelper.cpp"
